...
public:
	...
	void AppendPathDijkstra(CVertex* startVertex, CVertex* endVertex) {
		CTrack shortestPath(this->m_pGraph);
		CVertex* vActual = endVertex;
		while (vActual != startVertex) {
			shortestPath.AddFirst(vActual->m_pDijkstraPrevious);
			vActual = vActual->m_pDijkstraPrevious->m_pOrigin;
		}
		this->Append(shortestPath);
	}
};
...
CTrack BackTrack(CGraph& g, CVertex* pInici1, CVertex* pDesti1);
