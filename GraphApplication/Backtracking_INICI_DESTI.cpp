...
void BackTrack(NodeCami* pAnterior, CVertex* pActual, CVertex* pDesti1)
{
	if (pActual == pDesti1) {
		if (LongitudCamiActual < LongitudCamiMesCurt) {
			CamiMesCurt.Clear();
		...
		for (CEdge* pE : pActual->m_Edges) {
			if (!pE->m_pDestination->m_JaHePassat) {
				node.m_pEdge = pE;
				LongitudCamiActual += pE->m_Length;
				BackTrack(&node, pE->m_pDestination, pDesti1);
				LongitudCamiActual -= pE->m_Length;
				pE->m_pDestination->m_JaHePassat = false; // Restaurar el estado
			}
		}
		pActual->m_JaHePassat = false; // Restaurar el estado
	}
}
...
CTrack BackTrack(CGraph& g, CVertex* pInici1, CVertex* pDesti1)
{
	...
	LongitudCamiActual = 0.0;
	for (CVertex& v : g.m_Vertices) v.m_JaHePassat = false;
	BackTrack(NULL, pInici1, pDesti1);
	return CamiMesCurt;
}